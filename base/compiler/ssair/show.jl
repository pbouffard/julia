...
function ircode_new_nodes_iter(ir::IRCode)
    stmts = ir.stmts
...
function show_ir(io::IO, ir::IRCode, config::IRShowConfig=default_config(ir);
                 pop_new_node! = ircode_new_nodes_iter(ir))
...
function show_ir(io::IO, compact::IncrementalCompact, config::IRShowConfig=default_config(compact.ir))
    ...
    bb_idx = show_ir_stmts(io, compact, 1:compact.result_idx-1, config, used, cfg, 1; pop_new_node!)
    ...
    used = stmts_used(io, compact.ir)
    show_ir_stmts(io, compact.ir, compact.idx:length(stmts), config, used, cfg, bb_idx; pop_new_node!)
